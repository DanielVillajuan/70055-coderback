...
app.post('/recuperarPassword', async (req, res) => {
    ...
    const userFound = await UserModel.find({ email }).lean()
    userFound.password = createHash(password)
    const userUpdate = await UserModel.updateOne({_id: userFound._id},userFound).lean()

    res.status(203).json({ message:'cambio de contraseña hecha', user: userUpdate })
})
